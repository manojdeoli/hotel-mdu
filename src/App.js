...
function App() {
  ...
  const handleZoneScan = (zone, phoneNumber) => {
    ...
    const ZONES = {
      entrance: { name: 'Hotel Entrance', lat: -37.8136, lng: 144.9631 },
      checkin: { name: 'Check-in Kiosk', lat: -37.8137, lng: 144.9632 },
      elevator: { name: 'Elevator', lat: -37.8138, lng: 144.9633 },
      room: { name: 'Room 1337', lat: -37.8139, lng: 144.9634 }
    };

    const zoneData = ZONES[zone];
    if (!zoneData) return;

    setUserGps({ lat: zoneData.lat, lng: zoneData.lng });
    addMessage(`User detected at ${zoneData.name}`);

    // Trigger actions based on zone
    setTimeout(() => {
      if (zone === 'entrance') {
        addMessage('Welcome to Telstra Towers!');
      } else if (zone === 'checkin') {
        setCheckInStatus('Checked In');
        setRfidStatus('Verified');
        addMessage('Check-in complete!');
        setTimeout(() => setRfidStatus('Unverified'), 2000);
      } else if (zone === 'elevator') {
        checkIdentityIntegrity(false, 'Checked In');
  ...
  const validatePhone = async (e) => { // Made async to await api.locationRetrieval
    ...
    try {
      ...
      if (verificationData.devicePhoneNumberVerified === true) {
        ...
        localStorage.setItem('verifiedPhone', fullPhoneNumber);
      } else {
        setError(`Phone number verification failed.`);
      }
    } catch (err) {
      console.error('API call failed:', err);
      setError('An error occurred during verification. Please try again......Manoj');
    } finally {
      setIsLoading(false);
    }
  };
